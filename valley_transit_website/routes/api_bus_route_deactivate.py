...
async def handler(request: Request) -> HTTPResponse:
    ...
    if not current_status:
        return html('<script>alert("The route is not being operated.");</script>')
    route_status[f"route_{route}"] = False
    with open('./data/bus_route_status.json', 'w') as f:
        f.write(dumps(route_status))
    return html('<script>alert("The route has been deactivated.");</script>')